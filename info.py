...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '25617967').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002057898525').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '-1002172328535').split()]
...
# MongoDB information
DATABASE_URI = environ.get("DATABASE_URI", "mongodb+srv://satyajeetkumarofficial:Guriya50@cluster0.dmmijuh.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
SHORTLINK_API = environ.get('SHORTLINK_API', 'sk-proj-WPslhQWClzs2PdVr2gKUT3BlbkFJnw81cZgHA13VGbj3RppL')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002172328535')).split()]
